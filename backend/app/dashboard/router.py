...
from app.db.models import Exam, Persona, PersonaMemory, TeachingSession, WeakPointTag
...
async def home(
    ...
    mem_rows = (await db.scalars(select(PersonaMemory).where(PersonaMemory.persona_id == persona.id))).all()
    if mem_rows:
